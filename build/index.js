...
async function makeApiRequest(apiKey, endpoint, method = 'GET', body) {
    const options = {
        method,
        headers: {
...
// Register methods
server.tool('listPods', 'List all pods', ApiKeySchema.shape, async ({ apiKey }, extra) => {
    try {
...
server.tool('createPod', 'Create a new pod', { ...ApiKeySchema.shape, ...CreatePodInputSchema.shape }, async ({ apiKey, ...podConfig }, extra) => {
    try {
...
server.tool('getPod', 'Get information about a pod', { ...ApiKeySchema.shape, ...PodIdInputSchema.shape }, async ({ apiKey, podId }, extra) => {
    try {
        ...
        return {
            ...
            content: [
                {
                    type: 'text',
                    text: `Error getting pod: ${error instanceof Error ? error.message : String(error)}`,
                },
            ],
        };
    }
});
server.tool('terminatePod', 'Terminate a pod', { ...ApiKeySchema.shape, ...PodIdInputSchema.shape }, async ({ apiKey, podId }, extra) => {
    try {
        ...
        return {
            ...
            content: [
                {
                    type: 'text',
                    text: `Error terminating pod: ${error instanceof Error ? error.message : String(error)}`,
                },
            ],
        };
    }
});
server.tool('startPod', 'Start a stopped pod', { ...ApiKeySchema.shape, ...PodIdInputSchema.shape }, async ({ apiKey, podId }, extra) => {
    try {
        ...
        return {
            ...
            content: [
                {
                    type: 'text',
                    text: `Error starting pod: ${error instanceof Error ? error.message : String(error)}`,
                },
            ],
        };
    }
});
server.tool('stopPod', 'Stop a pod', { ...ApiKeySchema.shape, ...PodIdInputSchema.shape }, async ({ apiKey, podId }, extra) => {
    try {
        ...
        return {
            ...
            content: [
                {
                    type: 'text',
                    text: `Error stopping pod: ${error instanceof Error ? error.message : String(error)}`,
                },
            ],
        };
    }
});
server.tool('listServerlessEndpoints', 'List all serverless endpoints', ApiKeySchema.shape, async ({ apiKey }, extra) => {
    try {
